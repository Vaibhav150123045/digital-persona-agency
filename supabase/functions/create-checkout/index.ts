...
serve(async (req) => {
  ...
  try {
    const { tier = 'plus' } = await req.json();
    ...
    const pricing = {
      plus: { amount: 2299, name: "Plus Plan" }, // $22.99
      pro: { amount: 4299, name: "Pro Plan" }    // $42.99
    };

    const selectedPlan = pricing[tier as keyof typeof pricing] || pricing.plus;
